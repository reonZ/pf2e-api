...
class PredicatePF2e extends Array {
    ...
    #isTrue(statement, domain) {
        return ((typeof statement === "string" && domain.has(statement)) ||
    ...
    #testCompound(statement, domain) {
        return (("and" in statement && statement.and.every((s) => this.#isTrue(s, domain))) ||
            ("nand" in statement && !statement.nand.every((s) => this.#isTrue(s, domain))) ||
            ("or" in statement && statement.or.some((s) => this.#isTrue(s, domain))) ||
            ("xor" in statement &&
                statement.xor.filter((s) => this.#isTrue(s, domain)).length === 1) ||
...
class StatementValidator {
    ...
    static isBinaryOp(statement) {
        if (!foundry_api_1.R.isPlainObject(statement))
            return false;
        const entries = Object.entries(statement);
        if (entries.length > 1)
            return false;
        const [operator, operands] = entries[0];
        return (this.#binaryOperators.has(operator) &&
